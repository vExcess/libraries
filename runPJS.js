/*
    ...
    When this script loads it automatically searches for scripts that have the class name "pjs-src"
...
` + filling + `
    ...
    if (!document.getElementById("ka-pjs-source")) {
        pjsEnv.lib = document.createElement("script");
        pjsEnv.lib.id = "ka-pjs-source";
    ...
    if (code) {
        ...
        pjsEnv.canvas.id = pjsEnv.canvas.id || "pjs-canvas" + window.pjsEnvironments.length;
        if (options.placeholder) {
            options.placeholder.parentNode.replaceChild(pjsEnv.canvas, options.placeholder);
        } else {
        ...
        console.error("function createPJS(code, options*) did not recieve any input code");
    }
...
var pjsSources = document.getElementsByClassName("pjs-src");
for (var i = 0; i < pjsSources.length; i++) {
    let pjsSrc = pjsSources[i];
    
    // create canvas
    let canvas = document.createElement("canvas");
    pjsSrc.parentNode.replaceChild(canvas, pjsSrc);

    createPJS(pjsSrc.innerHTML, {
        width: pjsSrc.dataset.width || 400,
        height: pjsSrc.dataset.height || 400,
        canvas: canvas
    });
}
